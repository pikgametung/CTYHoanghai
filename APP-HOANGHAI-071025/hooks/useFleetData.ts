...
export const useFleetData = () => {
  ...
  const reconnectDelayRef = useRef<number>(2000); // Start with 2 seconds

  // Effect for real-time AIS data
  useEffect(() => {
    const connect = () => {
      if (wsRef.current) { // Don't try to connect if a connection is already active/opening
          return;
      }
      ...
      ws.onopen = () => {
          ...
          const subscriptionMessage = {
              ...
              MessageTypes: ["PositionReport"] // Be more specific about what we want
          };
      ...
      ws.onclose = (event) => {
          ...
          reconnectTimeoutRef.current = window.setTimeout(connect, currentDelay);
          
          reconnectDelayRef.current = Math.min(currentDelay * 2, 60000); // Double delay, max 60s
      };
    };
